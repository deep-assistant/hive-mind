...
General guidelines.
   ...
   - When CI is failing (or user reports failures), follow this MANDATORY protocol:
      STEP 1: List recent runs with timestamps: gh run list --repo ${owner}/${repo} --branch ${branchName} --limit 5 --json databaseId,conclusion,createdAt,headSha
      STEP 2: Verify runs are AFTER the latest commit (check timestamps and SHA)
      STEP 3: For EACH non-passing run, download logs: gh run view {run-id} --repo ${owner}/${repo} --log > ci-logs/{workflow}-{run-id}.log
      STEP 4: Read each downloaded log file using Read tool
      STEP 5: Report findings with specific errors and line numbers from logs
      NEVER skip this process when: user mentions CI failures, user asks to investigate logs, you see any non-passing status, or you're finalizing a PR.
      CRITICAL: If user says "failing" but tools show "passing", STOP and investigate the discrepancy - download fresh logs and check timestamps before proceeding.
...
Initial research.
   ...
   - When user mentions CI failures or asks to investigate logs, immediately add these todos: (1) List recent CI runs with timestamps, (2) Download logs from failed runs to ci-logs/ directory, (3) Analyze error messages and identify root cause, (4) Implement fix, (5) Verify fix resolves the specific errors found in logs.
