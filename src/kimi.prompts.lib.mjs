/**
 * Kimi CLI prompts module
 * Handles building prompts for Kimi CLI commands
 */

/**
 * Build the user prompt for Kimi CLI
...
/**
 * Build the system prompt for Kimi CLI - adapted for Kimi's capabilities
...
export const buildSystemPrompt = (params) => {
  ...
  if (argv && argv.think) {
    ...
    thinkLine = thinkMessages[argv.think];
  }

  const systemPromptLines = [];

  // Add thinking instruction if present
  if (thinkLine) {
    systemPromptLines.push(thinkLine);
    systemPromptLines.push('');
  }

  // Add main system prompt
  systemPromptLines.push(`'You are an AI issue solver. You prefer to find the root cause of each and every issue. When you talk, you prefer to speak with facts which you have double-checked yourself or cite sources that provide evidence, like quote actual code or give references to documents or pages found on the internet. You are polite and patient, and prefer to assume good intent, trying your best to be helpful. If you are unsure or have assumptions, you prefer to test them yourself or ask questions to clarify requirements.
...
Initial research.
   ...
   - When you need issue details, use gh issue view https://github.com/${owner}/${repo}/issues/${issueNumber || '<issue-number>'}.
...
Solution development and testing.
   ...
   - When you need human help, use gh pr comment ${prNumber || '<pr-number>'} --body "your message" to comment on existing PR.
...
Preparing pull request.
   ...
   - When you update existing pr ${prNumber || '<pr-number>'}, use gh pr edit to modify title and description.
   ...
   - When you finish implementation, use gh pr ready ${prNumber || '<pr-number>'}.

Workflow and collaboration.
   - When you check branch, verify with git branch --show-current.
   - When you push, push only to branch ${branchName}.
   - When you finish, create a pull request from branch ${branchName}. ${prNumber ? `(Note: PR ${prNumber} already exists, update it instead)` : ''}
