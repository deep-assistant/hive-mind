...
// Available model configurations for Codex
export const availableModels = {
  'codex': 'code-davinci-002',  // Default Codex model
  'codex-edit': 'code-davinci-edit-001', // Edit model
  'gpt-3.5-codex': 'gpt-3.5-turbo', // GPT-3.5 for code
};
...
export const validateCodexConnection = async (model = 'codex') => {
  ...
  const attemptValidation = async () => {
    try {
      if (retryCount === 0) {
        ...
        await log(`🔄 Retry attempt ${retryCount}/${maxRetries} for Codex CLI validation...`);
      }

      // Check if Codex CLI is installed and get version
      try {
        const versionResult = await $`timeout ${Math.floor(timeouts.claudeCli / 1000)} codex --version`;
      ...
      if (testResult.code !== 0) {
        ...
        if (stderr.includes('auth') || stderr.includes('login') || stderr.includes('api')) {
          ...
          await log('   💡 Please check your OpenAI API key configuration', { level: 'error' });
          return false;
        }
