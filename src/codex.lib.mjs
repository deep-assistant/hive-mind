...
export const validateCodexConnection = async (model = 'gpt-5') => {
  ...
  const attemptValidation = async () => {
    try {
      ...
      const testResult = await $`printf "echo hi" | timeout ${Math.floor(timeouts.codexCli / 1000)} codex exec --model ${mappedModel} --json --full-auto --dangerously-bypass-approvals-and-sandbox`;
...
export const executeCodexCommand = async (params) => {
  ...
  const executeWithRetry = async () => {
    ...
    let codexArgs = `exec --model ${mappedModel} --json --full-auto --dangerously-bypass-approvals-and-sandbox`;

    if (argv.resume) {
      // Codex supports resuming sessions
      await log(`🔄 Resuming from session: ${argv.resume}`);
      codexArgs = `exec resume ${argv.resume} --json --full-auto --dangerously-bypass-approvals-and-sandbox`;
    }
    ...
    try {
      ...
      if (argv.resume) {
        execCommand = $({
          cwd: tempDir,
          mirror: false
        })`cat ${promptFile} | ${codexPath} exec resume ${argv.resume} --json --full-auto --dangerously-bypass-approvals-and-sandbox`;
      } else {
        execCommand = $({
          cwd: tempDir,
          mirror: false
        })`cat ${promptFile} | ${codexPath} exec --model ${mappedModel} --json --full-auto --dangerously-bypass-approvals-and-sandbox`;
      }
